...
// btns.forEach(btn => console.log(btn.parentNode))
...
function handleClick(clickedButtonEvent) {
    ...
    const card = {
        ...
        productName: cardNode.getElementsByClassName('productName') , // здесь .productName, который внутри .product
    ...
    if (card.button[0].innerText == "Подробнее") { 
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card);
    } else if (card.button[0].innerText == "Отмена" ) {
...
function hideMoreText(card) {
    ...
    card.button[0].innerText = "Подробнее";
}
...
function showMoreText(card) {
    ...
    let description = 'Это описание товара';
    // 6.2 внутри .product есть .productName, после него вставляем div.desc c текстом из переменной из п. 6.1
    card.productName[0].insertAdjacentHTML('afterend',`<div class='desc'>${description}</div>`);
    // 6.3 внутри .product у кнопки текст ставим "Отмена"
    card.button[0].innerText = "Отмена";
}